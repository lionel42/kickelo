...
| Φ | Golden Streak | Player has the configured minimum count of 10:9 wins since their last 9:10 loss. Number = current qualifying streak. | `goldenPhiStreak` | ✅ implemented |
...
| 🦏 | Rhino (Shutout) | Match result | Player wins 10:0 (shutout). Each 10:0 victory adds 1 to the daily count. | `statusEvents.shutoutCount` | ✅ implemented |
...
| 🐧 | Chill Comeback | Match result | Win 10:9 after trailing the entire match until scoring the last two goals (tie at 9:9, then win). Each occurrence adds 1 for the day. | `statusEvents.chillComebackCount` | ✅ implemented |
